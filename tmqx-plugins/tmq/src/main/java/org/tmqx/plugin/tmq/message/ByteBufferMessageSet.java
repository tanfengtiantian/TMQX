...
import org.tmqx.common.IteratorTemplate;
...
import org.tmqx.plugin.tmq.exception.InvalidMessageSizeException;
import org.tmqx.plugin.tmq.exception.MessageSizeTooLargeException;
...
public class ByteBufferMessageSet extends MessageSet {
	...
	public static ByteBuffer createByteBuffer(CompressionCodec compressionCodec, TmqMessage... messages) {
        if (compressionCodec == CompressionCodec.NoCompressionCodec) {
            ByteBuffer buffer = ByteBuffer.allocate(messageSetSize(messages));
            for (TmqMessage message : messages) {
            	//设置size
	...
	public static int messageSetSize(TmqMessage... messages) {
        int size = 0;
        for (TmqMessage message : messages) {
	...
	public static int entrySize(TmqMessage message) {
	...
	class Iter extends IteratorTemplate<MessageAndOffset> {
		...
		private MessageAndOffset makeNextOuter() {
            ...
            TmqMessage newMessage = new TmqMessage(message);
            if(isShallow) {
