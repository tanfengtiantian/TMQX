...
public class FileMessageSet extends MessageSet{
	...
	private long validateMessage(FileChannel channel, long start, long len, ByteBuffer buffer) throws IOException {
        ...
        if (size < TmqMessage.MinHeaderSize) return -1;
        ...
        TmqMessage message = new TmqMessage(messageBuffer);
		...
		return new IteratorTemplate<MessageAndOffset>() {
            ...
            protected MessageAndOffset makeNext() {
                try {
                    ...
                    // 读到的消息长度大于 标题头的消息长度
                    if (size < TmqMessage.MinHeaderSize) {
                        return allDone();
                    }
                    //创建data缓冲区
                    ...
                    //offset 移动到下一条
                    location += size + 4;
                    return new MessageAndOffset(new TmqMessage(buffer), location);
                } catch (IOException e) {
