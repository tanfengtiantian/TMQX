...
import org.tmqx.plugin.tmq.exception.CompressionCodec;
import org.tmqx.plugin.tmq.exception.UnknownMagicByteException;
...
public class TmqMessage {
    ...
    public TmqMessage(ByteBuffer buffer) {
        this.buffer = buffer;
        this.messageSize = buffer.limit();
    }


    public TmqMessage(byte[] bytes) {
        this(bytes, CompressionCodec.NoCompressionCodec);
    }

    public TmqMessage(byte[] bytes, CompressionCodec compressionCodec) {
        this(Utils.crc32(bytes), bytes, compressionCodec);
    }


    public TmqMessage(long checksum, byte[] bytes, CompressionCodec compressionCodec) {
        //"magic" +  "attributes" + "CRC32" + N内容
        this(ByteBuffer.allocate(TmqMessage.headerSize(TmqMessage.CurrentMagicValue) + bytes.length));
        //1 byte "magic"  协议
    ...
    public static TmqMessage newInvalidMessage(Throwable cause) {
        return new TmqMessage(ByteBuffer.allocate(0));
    }
}
